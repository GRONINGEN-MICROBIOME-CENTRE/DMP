...
ANALYSIS 1: Run the GMHI classifier to predict disease status ("healthy" vs "unhealthy") based on our samples. We used a 90-10 training-testing split to generate a discovery (training data) and validation (testing data) cohort.
    ...
    Testing step 1b: Find the balance accuracy in the validation cohort that corresponds to the optimized theta_f and theta_d         found in the training step  
...
run_gmhi_classifier <- function(input_data = input_data) {
  ...
  training <- t(input_data) # taxa need to be on rows
  Healthy <- training[,colnames(training) %in% dat[dat$MED.DISEASES.None.No.Diseases %in% 1,]$sampleid]
  Nonhealthy <- training[,colnames(training) %in% dat[dat$MED.DISEASES.None.No.Diseases %in% 0,]$sampleid]
