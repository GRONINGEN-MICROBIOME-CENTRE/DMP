...
# 
# NOTES: 
# - this is implementation of scripts for MOCK DATA
# packaged with the DMP github repo. It is _not_ the real data
# used in the DMP paper and this code is intended for 
# demonstration/testing only

# - to reproduce Fig S3/a-b, download the taxonomy
# data from the EGA and use
# ClusterAnalysis_FigS3_EGA_MockPhenotypes.R
...
# ==================================
dag3=read.table("Mock_data/taxa.txt",sep = "\t",header = T,check.names = F,stringsAsFactors = F,row.names = 1)
...
print(g)
ggsave(plot = g, filename = 'microbiome_clustering/plots/MockData_FigS3a.png')
...
print(g)
ggsave(plot = g, filename = 'microbiome_clustering/plots/MockData_FigS3b.png')


# ===========================================
# associations between diseases and clusters
# ===========================================
metadata=read.table("Mock_data/diseases.txt")
disease=data.frame(Phenotype=colnames(metadata))
disease=disease[grep("MED",disease$Phenotype),,drop=F]
metadata=metadata[rownames(metadata) %in% rownames(clusters),]

disease_enterotype <-
  foreach(i=1:nrow(disease),.combine = rbind) %do%  {
    
    disease.sub=metadata[,colnames(metadata)==disease$Phenotype[i],drop=F]
    #cat(yellow(disease$Phenotype[i]),"\n")
    disease.sub=merge(disease.sub,clusters,by="row.names",all = F)
    disease.sub=na.omit(disease.sub)
    disease.sub[disease.sub=="Y"]=1
    disease.sub[disease.sub=="Y.always"]=1
    disease.sub[disease.sub=="Y.often"]=1
    disease.sub[disease.sub=="Y.sometimes"]=1
    disease.sub[disease.sub=="N"]=0
    disease.sub[,2]=as.numeric(disease.sub[,2])
    disease.sub[,3]=as.numeric(disease.sub[,3])
    disease.sub=na.omit(disease.sub)
    if(nrow(disease.sub)>100){
      fit_glm=glm(disease.sub[,2]~disease.sub[,3], family=binomial)
      pvalue=summary(fit_glm)$coef[2,4]
      or=or_glm(data = disease.sub, model = fit_glm, incr = 1)
      return.string = data.frame(Disease=disease$Phenotype[i],Pvalue=pvalue,OR=or[1,2],CI_up=or[1,4],CI_low=or[1,3])
    }
    
  }
disease_enterotype$FDR=p.adjust(disease_enterotype$Pvalue)
disease_enterotype$Sig=NA
disease_enterotype$Sig[disease_enterotype$FDR<0.05]="Significant"
disease_enterotype$Sig[disease_enterotype$FDR>=0.05]="Non-Significant"
...
disease_plot=subset(disease_enterotype,Sig=="Significant")

g <- ggplot(disease_enterotype, aes(x = OR, y = Disease,color=Sig)) + 
            geom_vline(aes(xintercept = 1), size = .25, linetype = "dashed") + 
            geom_errorbarh(aes(xmax = `CI_up`, xmin = `CI_low`), size = .3, height = 0.2) +
            geom_point(size = 3.5) +
            theme_bw()+ theme(panel.grid.minor = element_blank()) +
            ylab("") + xlab("Odds ratio")+scale_color_lancet()
print(g)
ggsave(plot = g, filename = 'microbiome_clustering/plots/MockData_FigS3c.png',dpi = 400,width=7)
