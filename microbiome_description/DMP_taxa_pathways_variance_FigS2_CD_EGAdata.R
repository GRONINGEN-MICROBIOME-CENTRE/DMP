...
#
# NOTES:
# - Plots require DMP microbiome data (avaliable at EGA, see manuscript for details)
# - setwd should be set to appropriate location (DMP folder of github repo)
...
# load EGA data
# ========================
inDFtaxa <- read.table('DMP_data_EGA/DMP_metaphlan2_raw.csv',sep=',',header=T)
inPh <- filterMetaGenomeDF(inDFtaxa,keepLevels = "P",keepDomains = c("Bacteria"),
                           presPerc = 0.01,minMRelAb = 0.0001,rescaleTaxa = T)
inDFpwys <- read.table('DMP_data_EGA/DMP_humann2_metacyc_raw.csv',sep=',',header=T)
inDFpwysNoID <- inDFpwys
inDFpwysNoID$ID <- NULL

pwyMeans <- colMeans(inDFpwysNoID)
pwyMeans <- pwyMeans[order(pwyMeans,decreasing = T)]

# TAXA plot
# ==============
rownames(inPh) <- inPh$ID; 
taxa <- inPh
...
g <- ggplot(taxaL,aes(x=ID,y=value,fill=variable,col=variable)) + geom_col() + theme_classic() + xlab('') +
  scale_fill_manual(values=colPal) + scale_color_manual(values=colPal) +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank()) + ylab('')
print(g)
ggsave(plot = g,filename = 'microbiome_description/plots/FigS2c_phyla_across_population.png',dpi = 600,width = 12,height = 5)
...
g <- ggplot(pwysL,aes(x=ID,y=value,fill=variable,col=variable)) + geom_col() + theme_classic() +
  scale_fill_manual(values=colPal) + scale_color_manual(values=colPal) + xlab('') +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank()) + ylab('')
print(g)
ggsave(plot = g,filename = 'microbiome_description/plots/FigS2d_Pwys_across_population.png',dpi = 600,width = 12,height = 5)
...
write.table(res,'microbiome_description/data/taxa_vs_pwys_variance_tests.csv',sep=',',row.names = F)
