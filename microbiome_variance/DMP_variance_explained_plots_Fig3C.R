...
# >> set working dir, working dir has to be DMP root folder, example:
#setwd('D:/Vbox/shared/dag/git_14_05/DMP/')
setwd('.') 

# input folder
inFldr <- "./microbiome_variance/group_adonis"
...
# prep output folder
if (!dir.exists('./microbiome_variance/group_adonis/plots_adonis/')) {
  dir.create('./microbiome_variance/group_adonis/plots_adonis/')
}
...
g
ggsave(plot = g,filename = "./microbiome_variance/group_adonis/plots_adonis/Fig_3c_adonis_taxa_pwys_seq_pergroup.png",width = 5,height = 8) 
write.table(inDFssPT,file = "./microbiome_variance/group_adonis/plots_adonis/Fig_3c_adonis_taxa_pwys_seq_pergroup_datatable.csv",sep=',',row.names = F)
...
g <- g + ylab("Beta-diversity variance explained (R2)") + theme(text = element_text(size = 14)) + ggtitle("") + xlab("Data layer")# + ylim(0,0.3)
g
ggsave(plot = g,filename = "./microbiome_variance/group_adonis/plots_adonis/adonis_taxa_pwys_seq_diseases.png",width = 5,height = 8) 
write.table(inDFssPT,file = "./microbiome_variance/group_adonis/plots_adonis/adonis_taxa_pwys_seq_pergroup_diseases.csv",sep=',',row.names = F)